...
const string MOVE_CHAR[MAX_DIR] = {"U","D","L","R"};
...
int		move_marble(string& p)
{
	...
	for(;!pq.empty();)
	{
		...
		for(int d=UP;d<=RIGHT;d++)
		{
			...
			if( red_exit == true )
			{
				p = moves[red_row][red_col][blue_row][blue_col]+MOVE_CHAR[d];
			...
			moves[prev_red_row][prev_red_col][prev_blue_row][prev_blue_col] = 
						moves[red_row][red_col][blue_row][blue_col]+MOVE_CHAR[d];
...
int		main(void)
{
	...
	min_value = move_marble(path);
	
	if(  min_value != -1 )
	{
		printf("%d\n%s\n",min_value,path.c_str());
	}
	else
	{
		puts("-1");
