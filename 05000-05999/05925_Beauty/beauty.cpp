...
int		get_min_distance(Pos& p,vector<Pos>& s)
{
	int		ret;
	int&	r1 = p.first;
	int&	c1 = p.second;
...
int		main(void)
{
	vector<Pos>	spot[3],empty;
	int			spot_ptr;
	
	input(empty);
	spot_ptr = 0;
	
	for(int row=0;row<N&&spot_ptr!=3;row++)
	{
		for(int col=0;col<M&&spot_ptr!=3;col++)
		{
	...
	int	d1,d2,d3,min_distance,max_distance;
	
	min_distance = INF;
	
	for(int i=0;i<empty.size();i++)
	{
		d1 = get_min_distance(empty[i],spot[0]);
		d2 = get_min_distance(empty[i],spot[1]);
		d3 = get_min_distance(empty[i],spot[2]);
		
		min_distance = min(min_distance,d1+d2+d3+1);
	}
	
	d1 = get_min_distance(spot[0],spot[1]);
	d2 = get_min_distance(spot[1],spot[2]);
	d3 = get_min_distance(spot[2],spot[0]);
	
	max_distance = max(d1,max(d2,d3));
	cout<<min(min_distance,d1+d2+d3-max_distance)<<'\n';
