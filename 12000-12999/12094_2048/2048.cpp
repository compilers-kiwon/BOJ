...
bool	compare_grid(int g1[][MAX_SIZE],int g2[][MAX_SIZE],int size)
{
	for(int row=1;row<=size;row++)
	{
		for(int col=1;col<=size;col++)
...
void	find_max_block(int num_of_operation,int& max_block,
					   int size,int grid[][MAX_SIZE])
{
	int	tmp;
	...
	for(int i=UP;i<=RIGHT;i++)
	{
		...
		if( compare_grid(grid,tmp,size) == false )
		{
			find_max_block(num_of_operation+1,max_block,size,tmp);
		}
	}
}
...
int		main(void)
{
	...
	max_block = 0;
	find_max_block(0,max_block,N,grid);
